...
-- Fix the get_shared_folders_for_user function to resolve the type mismatch
...
BEGIN
  ...
  SELECT 
    ...
    -- Cast the email to TEXT to ensure type consistency
    CAST((SELECT email FROM auth.users WHERE auth.users.id = fs.shared_by LIMIT 1) AS TEXT) as shared_by_email,
    fs.permission_level,
